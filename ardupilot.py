...
valid_tag_ids = {
                  0 : 32, # 8.5" x 11" tag black border size in mm
                  48 : 190, # need to be measured
                }
...
def send_distance_sensor_packet(tag, tag_size):
    ...
    uart.write(temp)
#    print("distance =")
#    print(min(max(int(z_to_mm(tag.z_translation(), tag_size) / 10), MAV_DISTANCE_SENSOR_min_distance), MAV_DISTANCE_SENSOR_max_distance))
...
def send_landing_target_packet(tag, w, h, tag_size):
    ...
    uart.write(temp)
 #  print("x=")
 #  print(((tag.cx() / w) - 0.5) * h_fov)
 #  print("y=")
 #  print(((tag.cy() / h) - 0.5) * v_fov)
    print("z =")
    print(min(max(z_to_mm(tag.z_translation(), tag_size) / 1000, MAV_LANDING_TARGET_min_distance), MAV_LANDING_TARGET_max_distance))
# Main Loop
